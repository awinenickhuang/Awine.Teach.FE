            ...
            case 'add':
                admin.popupRight({
                    ...
                    , success: function (layero, index) {
                        view(this.id).render('teachingaffairservice/course/add').done(function () {
                            form.render();
    ...
    table.on('tool(course-table)', function (obj) {
        ...
        if (obj.event === 'del') {
            ...
            admin.popupRight({
                ...
                , closeBtn: 1
                , success: function (layero, index) {
                    view(this.id).render('teachingaffairservice/course/edit', data).done(function () {
                        form.render();
                        //初始化老师
                        common.ajax(setter.apiAddress.aspnetuser.list, "GET", "", {}, function (res) {
                            $("#sel-teacher-list").append("<option value=\"\">请选择</option>");
            ...
            admin.popupRight({
                ...
                , closeBtn: 1
                , success: function (layero, index) {
                    view(this.id).render('teachingaffairservice/course/chargemanner', data).done(function () {
                        form.render();
                        //初始化课程收费方式列表
                        table.render({
