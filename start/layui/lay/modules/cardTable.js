layui.define(['table', 'laypage', 'jquery', 'element'], function (exports) {
    ...
    card.prototype.init = function () {
        ...
        if (url != null) {
            ...
            //获取数据
            var data = getData(url, option.headers);
            //格式化数据
    ...
    function createCard(elem, linenum, item, no) {
        ...
        card.push('<div id=' + item.id + '" class="layui-col-md' + line + ' ew-datagrid-item" data-index="' + no + '" data-number="1">');
        card.push('<div class="project-list-item">');
        card.push('<img class="project-list-item-cover" src="' + item.image + '">');
        card.push('<div class="project-list-item-body"> <h2>' + item.title + '</h2>');
        card.push('<div class="project-list-item-text layui-text">' + item.remark + '</div>');
        card.push('<div class="project-list-item-desc"> <span class="time">' + item.time + '</span>');
        card.push('<div class="ew-head-list"></div> </div>');
        card.push('</div>');
        card.push('</div>');
        card.push('</div>');
    ...
    function initData(tempData, option) {
        ...
        //转化数据和后端保持一致
        var resultData = tempData[option.response.dataName];
        var dataList = resultData.items;
        data.count = resultData.totalCount;
        data.data = [];
    ...
    exports(MOD_NAME, externalMethods);
})
