layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    */
    tableSelect.prototype.render = function (opt) {
        ...
        opt.searchPlaceholder = opt.searchPlaceholder || '查询关键字';
        ...
        elem.off('click').on('click', function (e) {
            ...
            var tableBox = '<div class="tableSelect layui-anim layui-anim-upbit" style="left:' + l + ';top:' + t + ';border: 1px solid #d2d2d2;background-color: #fff;box-shadow: 0 2px 4px rgba(0,0,0,.12);padding:15px 15px 5px 15px;position: absolute;z-index:999999999;margin: 5px 0;border-radius: 2px;">';
            tableBox += '<div class="tableSelectBar">';
            tableBox += '<form class="layui-form" action="" style="display:inline-block;">';
            tableBox += '<input style="display:inline-block;width:300px;height:30px;vertical-align:middle;margin-right:-1px;border: 1px solid #C9C9C9;" type="text" name="' + opt.searchKey + '" placeholder="' + opt.searchPlaceholder + '" autocomplete="off" class="layui-input"><button class="layui-btn layui-btn-sm layui-btn-primary tableSelect_btn_search" lay-submit lay-filter="tableSelect_btn_search"><i class="layui-icon layui-icon-search"></i></button>';
            tableBox += '</form>';
            tableBox += '<button style="float:right;" class="layui-btn layui-btn-sm tableSelect_btn_select"><i class="layui-icon layui-icon-ok"></i>确定</button>';
            tableBox += '</div>';
            ...
            function defaultChecked(res, curr, count) {
                if (opt.checkedKey && elem.attr('lay-selected-result')) {
                    var selected = elem.attr('lay-selected-result').split(",");
                    for (var i = 0; i < res.data.length; i++) {
            ...
            overWidth && tableBox.css({ 'left': 'auto', 'right': '15px' })
            ...
            function selectDone(checkStatus) {
                if (opt.checkedKey) {
                    ...
                    elem.attr("lay-selected-result", selected.join(","));
                }
